do
--developed by @hosein_fuckoff
local function create_group(msg)
  -- superuser and admins only (because sudo are always has privilege)
  if not is_admin(msg) then
    return "Você n?o é um Administrador."
  end
...
local function set_description(msg, data)
  ...
  return 'Descriç?o do grupo criada:\n\n'..deskripsi
end
--developed by @i9Dev
local function get_description(msg, data)
  local data_cat = 'description'
  if not data[tostring(msg.to.id)][data_cat] then
    return 'O grupo n?o tem uma descriç?o.'
	end
...
local function export_chat_link_callback(cb_extra, success, result)
  ...
  if success == 0 then
    return send_large_msg(receiver, "N?o é poss?vel gerar link para este grupo.")
  end
